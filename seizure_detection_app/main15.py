...
def predict_all_windows(raw_obj, model, window_sec=3.0, overlap_sec=1.0):
    ...
    while (start_samp + w_samps) <= total_samples:
        ...
        seg_tensor = torch.tensor(data[:, start_samp:(start_samp+w_samps)], dtype=torch.float32).unsqueeze(0)
        with torch.no_grad():
...
def create_sliding_gif(raw_obj, intervals, picks=None, scale='norm', fps=10, dynamic_offset=True):
    """
    Revised function to generate an auto-playing Plotly animation of the sliding window.
    Instead of generating a GIF with Matplotlib, it returns a Plotly figure that auto-plays on page load.
    """
    import numpy as np
    ...
    # Decimate data if needed for plotting performance
    max_points = 2000
    dec_factor = max(1, len(times) // max_points)
    t_plot = times[::dec_factor]
    ...
    # Calculate base offset for stacking channels
    if dynamic_offset:
    ...
    # Define y-axis range based on offsets
    if picks:
        v_min = -base_offset * 0.5
    ...
    # Base layout with auto-play button and hidden slider controls
    layout = go.Layout(
        ...
        sliders=[{
            ...
            "pad": {"b": 10, "t": 50},
            "len": 0.9,
            "visible": False,  # hide slider controls for a cleaner look
    ...
    # Create frames where each frame adds one more rectangle corresponding to an interval
    frames = []
    for i in range(len(intervals)):
        # Build shapes for intervals up to the current frame
        frame_shapes = []
        for j in range(i + 1):
    ...
    # Auto-play configuration and layout tweaks
    fig.update_layout(
        transition={"duration": 0},
        autosize=True,
...
def main():
    ...
    model_opt = st.sidebar.selectbox("Model Type", ["EEGCNN", "EEGTransformerCNN"], key="model_opt_select")
    if model_opt == "EEGCNN":
        st.sidebar.write("Using EEGCNN – must match training shape.")
        f_eegcnn = st.sidebar.file_uploader("EEGCNN .pth", type=["pth", "pt"], key="eegcnn_file")
        if f_eegcnn:
            ...
            try:
                ...
                st.sidebar.success("EEGCNN loaded!")
            ...
            with st.expander("Multi-Channel Overview (Original)", expanded=True):
                ...
                if picks:
                    fig = plot_entire_eeg_multichannel_two_color(
                        ro, picks, red_intervals=None, green_intervals=None,
        ...
        if st.session_state.raw_montaged is None:
            ...
            if st.session_state.raw_bandpassed is not None:
                ...
                with st.expander("Bandpassed Multi-Channel Overview", expanded=True):
                    all_chs = list(range(len(ro.ch_names)))
                    picks = st.multiselect("Channels (BP)", all_chs, default=all_chs,
                                           format_func=lambda i: ro.ch_names[i])
    ...
    # STEP 4: Notch
    with tabs[3]:
        st.subheader("4. Notch Filter (Default=70)")
        if st.session_state.raw_bandpassed is None:
            st.warning("Finish step 3.")
        else:
            notch_str = st.text_input("Notch freq (comma e.g. 50,60)", "70")
            ...
            if st.session_state.raw_notched is not None:
                ...
                with st.expander("Notched Multi-Channel Overview", expanded=True):
                    all_chs = list(range(len(ro.ch_names)))
                    picks = st.multiselect("Channels (Notched)", all_chs, default=all_chs,
                                           format_func=lambda i: ro.ch_names[i])
                    ...
                    if picks:
                        ...
                        st.plotly_chart(fignt, use_container_width=True)
        ...
        if st.session_state.raw_notched is None:
            ...
            if st.session_state.raw_normalized is not None:
                ...
                with st.expander("Normalized Multi-Channel Overview", expanded=True):
                    ...
                    decim = st.checkbox("Decimate Data in Plot?", value=False)
                    if picks:
        ...
        if st.session_state.model is None:
            ...
            with st.expander("6.1 Predict All Windows", expanded=False):
                with st.form(key="predict_all_windows"):
                    ...
                    if submit1:
                        intervals = predict_all_windows(raw_obj, st.session_state.model,
                        ...
                        st.success(f"Finished predictions! Raw seizure segments: {n_seiz}, total raw duration: {dur_seiz:.2f}s (overlap included).")

            # Panel 6.2: Prediction Animation
            with st.expander("6.2 Prediction process (Cumulative)", expanded=False):
                with st.form(key="prediction_animation"):
                    gif_fps = st.number_input("Play FPS", 1, 30, 10)
                    submit2 = st.form_submit_button("Generate Animation")
                    if submit2:
                        if not st.session_state.window_preds:
                            st.info("No predictions yet, run 6.1.")
                        else:
                            st.info("Generating prediction animation, please wait...")
                            anim_fig = create_sliding_gif(raw_obj,
                                                          st.session_state.window_preds,
                                                          picks=None,
                                                          scale='norm',
                                                          fps=gif_fps,
                                                          dynamic_offset=True)
                            st.success("Animation created! Displaying below:")
                            st.plotly_chart(anim_fig, use_container_width=True)

            # Panel 6.3: Post-process & Final Plotly
            with st.expander("6.3 Post-process & Final Plotly", expanded=False):
                with st.form(key="merge_complement"):
                    ...
                    if submit3:
                        ...
                        st.session_state.pred_seizure_intervals = merge_intervals(raw_seiz_list, mgap)
                        st.session_state.pred_nonseizure_intervals = complement_intervals(total_dur, st.session_state.pred_seizure_intervals)
                        st.success("Merged intervals done!")
                        seg_count = len(st.session_state.pred_seizure_intervals)
                        dur_s = sum(d for (_, d) in st.session_state.pred_seizure_intervals)
                        st.write(f"Merged seizure segments: {seg_count}, total merged duration: {dur_s:.2f}s (no overlap).")
                        show_predict_info_sidebar()

                if st.session_state.pred_seizure_intervals or st.session_state.pred_nonseizure_intervals:
                    ...
                    fig_final = plot_entire_eeg_multichannel_two_color(
                        raw_obj, all_chs,
                        red_intervals=st.session_state.pred_seizure_intervals,
                        green_intervals=st.session_state.pred_nonseizure_intervals,
                        offset_factor=300.0,
                        decimate=dec,
                        dynamic_offset=True,
                        scale='norm',
            ...
            # Panel 6.4: Final Snippet (Matplotlib)
            with st.expander("6.4 Snippet (Matplotlib)", expanded=False):
                with st.form(key="final_snippet"):
                    dur_ = st.slider("Snippet Dur (Final Pred)", 1.0, min(30.0, raw_obj.times[-1]), 5.0)
                    stt_ = st.slider("Snippet Start (Final Pred)", 0.0, float(raw_obj.times[-1]-dur_), 0.0)
                    submit4 = st.form_submit_button("Plot Final Prediction Snippet")
                    if submit4:
                        sfig = plot_snippet_matplotlib(
                            raw_obj, stt_, dur_,
                            intervals_red=st.session_state.pred_seizure_intervals,
                            intervals_green=st.session_state.pred_nonseizure_intervals,
                            title="Final Pred Snippet (Red=Seiz, Green=NonSeiz)"
                        )
                        if sfig:
                            st.pyplot(sfig)
                    else:
